...
| L8 | features → student | **WORKS** | retrained 2026-09-12: **92.7 % ns-3**, 95.8 % refsim, 89.3 % held-out closed loop, 93.8 % under mutation |
...
| **G5** | `reactive` weak in the closed loop (student 97.8 % under mutation but LLM 0/2 live) | Feedback pathology: the agent's own probing changes the statistic it reads | AGENTIC_PLAN.md Week 3 |
...
| **G6** | `sweep` does not generalise when held out (0 % vs ~85 % trained) | Honest negative result — novel attacks are not solved | reported as a limitation, not hidden |
