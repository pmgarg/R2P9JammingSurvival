...
def main():
    ...
    with open(hp, "w") as fh:
        ...
        # AUDIT F4.6: this read `W_int8` unconditionally while the mixed-precision path
        # above stores fp32 `W` for a head that fails the equivalence gate -- so the
        # header generator raised KeyError and left a 162-byte stub that the report then
        # described as "emitted". Emit whichever precision the head actually shipped.
        for head in ("cause", "call"):
            if "W_int8" not in qbundle[head]:
                fh.write(f"\n/* {head}: kept fp32 (failed the equivalence gate) */\n")
                for li, w in enumerate(qbundle[head]["W"]):
                    arr = np.asarray(w, dtype=np.float32)
                    fh.write(f"static const float {head}_w{li}[{arr.size}] = {{")
                    fh.write(",".join(f"{v:.9g}f" for v in arr.flatten()))
                    fh.write("};\n")
                for li, b in enumerate(qbundle[head]["b"]):
